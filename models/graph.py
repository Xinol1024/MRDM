...
# from .kan_layer import KanGNN as KANLayer
...
class NodeBlock(Module):
    ...
    def __init__(self, node_dim, edge_dim, hidden_dim, use_gate):
        ...
        # self.node_net = KANLayer(node_dim, hidden_dim, hidden_dim)
        # self.edge_net = KANLayer(edge_dim, hidden_dim, hidden_dim)
        self.node_net = EKAN(node_dim, hidden_dim, hidden_dim, base_activation=nn.Identity)
        self.edge_net = EKAN(edge_dim, hidden_dim, hidden_dim, base_activation=nn.Identity)
        self.msg_net = Linear(hidden_dim, hidden_dim)

        if self.use_gate:
            # self.gate = MLP(edge_dim+node_dim+1, hidden_dim, hidden_dim) # add 1 for time
            # self.gate = KANLayer(edge_dim+node_dim+1, hidden_dim, hidden_dim) # add 1 for time
            self.gate = EKAN(edge_dim+node_dim+1, hidden_dim, hidden_dim, base_activation=nn.Identity)
...
class BondFFN(Module):
    def __init__(self, bond_dim, node_dim, inter_dim, use_gate, out_dim=None):
        ...
        # self.inter_module = KANLayer(inter_dim, out_dim, inter_dim)
        self.inter_module = EKAN(inter_dim, out_dim, inter_dim, base_activation=nn.Identity)
        if self.use_gate:
            # self.gate = MLP(bond_dim+node_dim+1, out_dim, 32)  # +1 for time
            # self.gate = KANLayer(bond_dim+node_dim+1, out_dim, 32)  # +1 for time
            self.gate = EKAN(bond_dim+node_dim+1, out_dim, 32, base_activation=nn.Identity)
...
class PosUpdate(Module):
    def __init__(self, node_dim, edge_dim, hidden_dim, use_gate):
        ...
        # self.left_lin_edge = KANLayer(node_dim, edge_dim, hidden_dim)
        self.left_lin_edge = EKAN(node_dim, edge_dim, hidden_dim, base_activation=nn.Identity)
        # self.right_lin_edge = KANLayer(node_dim, edge_dim, hidden_dim)
        self.right_lin_edge = EKAN(node_dim, edge_dim, hidden_dim, base_activation=nn.Identity)
