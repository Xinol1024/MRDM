...
class ExponentialRangeScoreModifier(ScoreModifier):
    """
    Input scores between [lower_bound, upper_bound] are scored as 1,
    otherwise they are smoothly decreased to 0 using an exponential function.
    """

    def __init__(self, lower_bound=2.0, upper_bound=3.0, decay_rate=0.5) -> None:
        """
        Args:
            lower_bound: The lower bound of the range with the highest score.
            upper_bound: The upper bound of the range with the highest score.
            decay_rate: The exponentially decreasing decay rate.
        """
    ...
    def __call__(self, x):
        if self.lower_bound <= x <= self.upper_bound:
            ...
            # 在 lower_bound 左侧使用指数函数递减
            return np.exp(-self.decay_rate * (self.lower_bound - x))
        else:
            # 在 upper_bound 右侧使用指数函数递减
            return np.exp(-self.decay_rate * (x - self.upper_bound))
